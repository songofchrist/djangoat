...
def assign_fields(obj, dct, fields, default=None):
    """
    :param obj: object to which to assign fields
    :param dct: dictionary from which to retrieve values based on "fields"
    :param fields: like-named fields to assign; can also be a tuple of (OBJ_FIELD, DICT_KEY), in which case DICT_KEY
        can be a key or string of "." delimited keys, denoting deeper values. For example, "one.two.three" will retrieve
        the value at dct["one"]["two"]["three"]. If at any point the key doesn't exist, we'll use the default value.
    :param default: what to use if the field doesn't exist
    """
    for f in fields:
...
def get_csv_content(rows, dialect=csv.excel, keys=None):
    """
    :param rows: a list of lists, each of which contains a row of data to output as a CSV, or a list of dicts or
        OrderedDicts, whose keys will be used as column headings. For example:
        ...
        For the first format, column headers should be included as the first row of data. For the second, headers
        will be derived either from "fieldnames" or the first row's keys.
    ...
    :return: content, formatted in CSV format
    """
    f = StringIO()
    if not (isinstance(rows[0], list) or isinstance(rows[0], tuple)):  # a list of dicts / OrderedDicts
        if not keys:
            keys = list(rows[0].keys())
        rows = get_row_values(rows, keys)
...
def get_csv_file(filename, rows, dialect=csv.excel, keys=None):
    """
    :param filename: the name of the CSV file, without the ".csv" extension
    :param rows: see "get_csv_content" for acceptable formats
    :param dialect: the dialect in which to write the CSV
    :param keys: when "rows" is a list of dicts or OrderedDicts, the keys of the values to include in the output
    :return: a CSV file
    """
    return HttpResponse(
        get_csv_content(rows, dialect, keys),
...
def get_csv_rows_from_queryset(queryset, values, prettify_headers=False, derived_fields=None, dynamic_columns=None):
    ...
    :param values: a tuple or list of the following form:
            ...
            (
                ...
                ('choice_field', 'Choice Field', {VALUE1: TEXT1, VALUE2: TEXT2 . . .}),
                ('method', 'Method With Custom Title')
                ('rep__user__name', 'Auto Annotated User Name'),
                ('_annotated__avg', 'Annotated Average'),
            )

        Field names in this scheme may either refer to fields or methods on queryset instances, a field on another
...
def get_json_file(filename, rows, keys=None):
    """
    :param filename: the name of the JSON file, without the ".json" extension
    :param rows: see "get_csv_content" for acceptable formats
    :param keys: if provided, "rows" is assumed to be a list of dicts or OrderedDicts, and we'll get the corresponding values
    :return: a JSON file
    """
    return HttpResponse(
        json.dumps(get_row_values(rows, keys) if keys else rows, sort_keys=True, indent=2),
...
def get_row_values(rows, keys):
    """
    :param rows: a list of dicts / OrderedDicts
    :param keys: the keys of the values in each dict whose values to include in the resulting array
    :return: a list of lists, suitable for output as a CSV
    """
    return [keys] + [[r[n] for n in keys] for r in rows]
...
def get_xls_file(filename, rows, keys=None):
    """
...
def get_xlsx_file(filename, rows, keys=None):
    """
